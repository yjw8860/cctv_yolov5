...
from utils.frequency import getFrequency
...
def test(data,
        ...
        for si, pred in enumerate(output):
            ...
            if save_txt:
                ...
                for *xyxy, conf, cls in predn.tolist():
                    ...
                    line = (cls, conf, *xywh)
                    txt_save_path = os.path.join(pred_result_dir, path.stem + '.txt')
    ...
    if verbose and nc > 1 and len(stats):
        ...
        stat_df = pd.DataFrame({'Class':cls_list, 'The number of images':img_list,
                                'Targets':target_list, 'AP':mAP50_list}, columns=['Class', 'The number of images', 'Targets', 'AP'])
...
def get_current_enviroment():
    ...
    df.to_csv('./Hardware_information.csv', index=False, encoding='euc-kr')
...
if __name__ == '__main__':
    ...
    getFrequency = getFrequency(config)
    getFrequency.getClassFreq()
    getFrequency.getClassFreq(is_valid=True)

    convert_data_path(DIR)

    print(f'save_json = {opt.save_json}')
    ...
    if opt.task in ['val', 'test']:  # run normally
        test(opt.data,
             ...
             pred_result_dir = DIR['ROOT_DIR']
             # save_conf=opt.save_conf,
             )
        print('test finished')
    #
    # intersect_gt_and_dr(DIR)
    # get_IoU(DIR=DIR)